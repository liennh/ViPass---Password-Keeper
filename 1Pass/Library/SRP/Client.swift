...
public class Client {
        ...
        privateKey: Data? = nil)
    {
        ...
        if let privateKey = privateKey {
            ...
            let bytes = try! Random.generate(byteCount: 128)
            a = BigUInt(Data(bytes: bytes))
        }
    ...
    public func processChallenge(salt: Data, publicKey serverPublicKey: Data) -> (Bool, String)  {
        ...
        guard B % N != 0 else {
            return (false, "Invalid server public key.")
        }
        ...
        let value1 = (B + N - k * v % N)
        let value2 = a + u * x
        let S = value1.power(value2, modulus: N)
        ...
        return (true, "Processed challenge successfully.")
    }
    
    // This func is used only for SignUp. Not used for SignIn. Made by Alex Ngo
    public func calculateSessionKey(serverPublicKey: Data) -> (Bool, Any) {
        let H = Digest.hasher(algorithm)
        let N = group.N
    ...
    public func verifySession(keyProof serverKeyProof: Data) -> (Bool, String) {
        guard let HAMK = HAMK else {
            return (false, "Missing challenge.")
        }
        guard HAMK == serverKeyProof else {
            return (false, "Key proof mismatch.")
        }
        isAuthenticated = true
        return (true, "Session Key is available. Login was successful.")
    }
